...
AS 
	...
	BEGIN
		INSERT INTO [Configuration].[RegisteredDatabases]
				SELECT  [Sdb].[name]
				FROM    [sys].[databases] Sdb
						LEFT OUTER JOIN [Configuration].[RegisteredDatabases] Rdb ON [Sdb].name = Rdb.DatabaseName
				WHERE   Rdb.DatabaseName IS NULL
    ...
    IF EXISTS ( SELECT   [Rdb].[DatabaseName]
				...
				AND @Purge = 0 
        BEGIN
            RAISERROR('Registered database(s) no longer exists. If any configurations are pointing to this database they will fail.',16,1)
        END	
