/*
PROCEDURE:		[DiskCleanup].[usp_BackupOptionAssociation]
AUTHOR:			Derik Hammer
CREATION DATE:	5/2/2012
DESCRIPTION:	This procedure is used to associate or disassociate an existing disk cleanup option to an existing umbrella configuration.
...
*/
CREATE PROCEDURE [DiskCleanup].[usp_DiskCleanupOptionAssociation]
	@debug BIT = 0,
	@OptionID INT,
	@ConfigID INT,
	@IsEnabled BIT,
...
AS
	...
	BEGIN TRANSACTION
		...
		IF NOT EXISTS	(
							SELECT [DiskCleanupConfigID] 
							FROM [DiskCleanup].[Configs] 
		...
		BEGIN
			...
			BEGIN  
				...
				INSERT INTO [DiskCleanup].[Configs]
						([OptionID]
						,[ConfigID]
						,[IsEnabled])
					VALUES
				...
				SELECT @LogEntry = 'Disk Cleanup OptionID ' + CAST(@OptionID AS VARCHAR(10)) + ' is now associated with Configuration ID ' + CAST(@ConfigID AS VARCHAR(10)) + ' and is ' + CASE CAST(@IsEnabled AS CHAR(1)) WHEN '1' THEN 'enabled.' ELSE 'disabled.' END
		...
		BEGIN
			...
			BEGIN 
				...
				DELETE FROM [DiskCleanup].[Configs] WHERE [OptionID] = @OptionID AND [ConfigID] = @ConfigID

				SELECT @LogEntry = 'Disk Cleanup OptionID ' + CAST(@OptionID AS VARCHAR(10)) + ' has been disassociated with Configuration ID ' + CAST(@ConfigID AS VARCHAR(10))
				EXEC Logging.usp_InsertLogEntry @Feature = 'Disk Cleanup', @TextEntry = @LogEntry, @LogMode = 'LIMITED'
