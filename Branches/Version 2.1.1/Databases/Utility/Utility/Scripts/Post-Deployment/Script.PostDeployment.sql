...
--------------------------------------------------------------------------------------
*/
PRINT 'Dropping and Creating Job [Utility - Validate Backup Plans]'
...
IF '$(Environment)' = 'QA' OR '$(Environment)' = 'PROD'
BEGIN
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Utility - Validate Backup Plans', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=2, 
		@notify_level_netsend=0, 
...
BEGIN
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'EXEC [Backup].[usp_VerifyBackupPlansExist]', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'EXEC [Utility].[Backup].[usp_VerifyBackupPlansExist] @debug = 1, @MailProfile = ''DefaultMailProfile''', 
		@database_name=N'Utility', 
		@flags=0
END 
...
--------------------------------------------------------------------------------------
*/
PRINT 'Dropping and recreating Job [Utility - Purge Log Entries]'
USE [msdb]
GO
...
IF '$(Environment)' = 'QA' OR '$(Environment)' = 'PROD'
BEGIN
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Utility - Purge Log Entries', 
			@enabled=1, 
			@notify_level_eventlog=0, 
			@notify_level_email=2, 
			@notify_level_netsend=0, 
...
END       
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Purge Log Entries]    Script Date: 12/16/2012 21:52:41 ******/
