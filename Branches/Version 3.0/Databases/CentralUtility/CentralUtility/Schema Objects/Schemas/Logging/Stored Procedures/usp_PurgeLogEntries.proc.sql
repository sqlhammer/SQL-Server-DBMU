	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SET @LogEntry = 'Purged oldest ' + CAST(@RowDiff AS VARCHAR(10)) + ' records from [Logging].[Entries] table.'
		...
		BEGIN
			...
			SET @LogEntry = 'DELETE FROM [Logging].[Entries] WHERE [InsertDate] < ' + CAST(@DateMarker AS VARCHAR(25))
			EXEC Logging.usp_InsertLogEntry @Feature = 'Logging', @TextEntry = @DynamicSQL, @LogMode = 'VERBOSE'
			
			DELETE FROM [Logging].[Entries]
			WHERE [InsertDate] < @DateMarker

			SET @LogEntry = 'Purged records older than ' + CAST(@DateMarker AS VARCHAR(25)) + ' from [Logging].[Entries] table.'
			EXEC Logging.usp_InsertLogEntry @Feature = 'Logging', @TextEntry = @LogEntry, @LogMode = 'LIMITED'
		END
