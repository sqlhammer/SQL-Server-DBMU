/*
PROCEDURE:		[IndexMaint].[usp_DisableConfig]
AUTHOR:			Derik Hammer
CREATION DATE:	10/16/2013
DESCRIPTION:	This procedure is used to disable IndexMaint feature level configurations.
...
*/
CREATE PROCEDURE [IndexMaint].[usp_DisableConfig]
	@OptionID int,
...
AS
	...
	IF NOT EXISTS (SELECT OptionID FROM [IndexMaint].[Configs] WHERE OptionID = @OptionID)
	BEGIN
	...
	END
	-- - verify configid
	IF NOT EXISTS (SELECT ConfigID FROM [IndexMaint].[Configs] WHERE ConfigID = @ConfigID)
		AND @ConfigID IS NOT NULL
	BEGIN
	...
	IF NOT EXISTS (SELECT ConfigName FROM [Configuration].[Configs] configs
					INNER JOIN [IndexMaint].[Configs] bakConfigs ON bakConfigs.ConfigID = configs.ConfigID
