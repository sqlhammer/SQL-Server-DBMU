...
AS
	...
	BEGIN
		...
		BEGIN
			PRINT '-------------------------------------------------------------------------'
			PRINT 'Cursor Iteration for @DiskCleanupOptionsID: ' + CAST(@DiskCleanupOptionsID AS VARCHAR(128))
			PRINT '-------------------------------------------------------------------------'
		END  
		...
		BEGIN
			...
			BEGIN
				PRINT '-------------------------------------------------------------------------'
				PRINT 'Cursor Iteration for @DBName: ' + CAST(@DBName AS VARCHAR(128))
				PRINT '-------------------------------------------------------------------------'
			END   
			...
			BEGIN
				...
				SET @DeleteDate = ( SELECT  REPLACE(CONVERT(NVARCHAR(30), @DeleteDateTime, 111),
													'/', '-') + 'T'
											+ CONVERT(NVARCHAR(30), @DeleteDateTime, 108)
								  )

				IF @debug = 1
				BEGIN
					PRINT '@DeleteDate = ' + CAST(@DeleteDate AS varchar(128))
					PRINT '@Extension = ' + @Extension
					PRINT '@Path = ' + @FilePath
				END 

				SET @LogEntry = 'Attempting execution of: EXECUTE master.dbo.xp_delete_file 0, ''' + @FilePath + ''',  + ''' + @Extension + ''', ''' + CAST(@DeleteDate AS VARCHAR(30)) + ''', 1'
				...
				SET @LogEntry = 'Execution success for: EXECUTE master.dbo.xp_delete_file 0, ''' + @FilePath + ''',  + ''' + @Extension + ''', ''' + CAST(@DeleteDate AS VARCHAR(30)) + ''', 1'
				EXEC Logging.usp_InsertLogEntry @Feature = 'Disk Cleanup', @TextEntry = @LogEntry, @LogMode = 'VERBOSE'
			END  
