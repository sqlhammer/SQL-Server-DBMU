...
AS
	...
	DECLARE @CurrentPreferOnline BIT
	...
	DECLARE @RequestedPreferOnline BIT
	--Logging
	...
	BEGIN TRANSACTION 
		...
		SELECT	@CurrentFragLimit = [FragLimit],
				...
				@CurrentPreferOnline = [PreferOnline]
		FROM [IndexMaint].[Options]
		WHERE [OptionID] = @OptionID
		...
		BEGIN
			...
			PRINT '--PreferOnline = ' + CAST(@CurrentPreferOnline AS VARCHAR)
			PRINT '------------------------------------------------------------'
			PRINT ''
		END
		...
		--Set/Validate FragLimit
		IF @PreferOnline IS NOT NULL
		BEGIN      
			SET @RequestedPreferOnline = @PreferOnline

			IF @debug = 1
			BEGIN
				PRINT 'Population of @RequestedPreferOnline, derived from @PreferOnline, complete.'
			END
		...
		ELSE
		BEGIN      
			SET @RequestedPreferOnline = @CurrentPreferOnline

			IF @debug = 1
			BEGIN
				PRINT 'Population of @RequestedPreferOnline, derived from @CurrentPreferOnline, complete.'
			END
		...
		SET @UpdateStatement =	'UPDATE [IndexMaint].[Options]' +
			...
			', [PreferOnline] = ' + CAST(@RequestedPreferOnline AS VARCHAR) +
		...
		BEGIN
			...
			PRINT '--PreferOnline = ' + CAST(@RequestedPreferOnline AS VARCHAR)
			PRINT '---------------------------------------------------------------'
			PRINT ''
