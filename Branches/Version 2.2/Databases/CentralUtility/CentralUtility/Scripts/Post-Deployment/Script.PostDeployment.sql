...
/****************************************************/
/*             Populate Lookup._Purpose             */
DECLARE @h_Purpose INT

DECLARE @xmlPurpose XML = N'
<purposes>
  <purpose Purpose="Active Directory"/>
  <purpose Purpose="Application"/>
  <purpose Purpose="Content"/>
  <purpose Purpose="Build Machine"/>
  <purpose Purpose="Database"/>
  <purpose Purpose="FTP"/>
  <purpose Purpose="Network"/>
  <purpose Purpose="Practice"/>
  <purpose Purpose="Storage"/>
  <purpose Purpose="Team Foundation"/>
  <purpose Purpose="Terminal"/>
  <purpose Purpose="Tax Prep Software"/>
  <purpose Purpose="Web"/>
</purposes>
'

EXEC sys.sp_xml_preparedocument @h_Purpose OUTPUT, @xmlPurpose

MERGE [Lookup].[_Purpose] AS t USING (
	SELECT p.Purpose
	FROM OPENXML(@h_Purpose, '/purposes/purpose', 1)
	WITH (Purpose VARCHAR(255)) AS p) AS s (Purpose)
ON (s.Purpose = t.Purpose)
WHEN MATCHED THEN
	UPDATE SET Purpose = s.Purpose
WHEN NOT MATCHED BY TARGET THEN
	INSERT (Purpose) VALUES (s.Purpose)
;
/*             Populate Lookup._Purpose             */
/****************************************************/

/****************************************************/
/*           Populate Lookup._Environment           */
DECLARE @h_Environment INT

DECLARE @xmlEnvironment XML = N'
<environments>
  <environment Environment="Archives"/>
...
EXEC sys.sp_xml_preparedocument @h_Environment OUTPUT, @xmlEnvironment

MERGE [Lookup].[_Environment] AS t USING (
	SELECT p.Environment
	FROM OPENXML(@h_Environment, '/environments/environment', 1)
	WITH (Environment VARCHAR(255)) AS p) AS s (Environment)
ON (s.Environment = t.Environment)
WHEN MATCHED THEN
	UPDATE SET Environment = s.Environment
WHEN NOT MATCHED BY TARGET THEN
	INSERT (Environment) VALUES (s.Environment)
;
/*           Populate Lookup._Environment           */
/****************************************************/

/****************************************************/
/*             Populate Lookup._Location            */
DECLARE @h_Location INT

DECLARE @xmlLocation XML = N'
<locations>
  <location Location="Cloud"/>
  <location Location="Toronto"/>
  <location Location="Virginia Beach"/>
</locations>
'

EXEC sys.sp_xml_preparedocument @h_Location OUTPUT, @xmlLocation

MERGE [Lookup].[_Location] AS t USING (
	SELECT p.Location
	FROM OPENXML(@h_Location, '/locations/location', 1)
	WITH (Location VARCHAR(255)) AS p) AS s (Location)
ON (s.Location = t.Location)
WHEN MATCHED THEN
	UPDATE SET Location = s.Location
WHEN NOT MATCHED BY TARGET THEN
	INSERT (Location) VALUES (s.Location)
;
/*             Populate Lookup._Location            */
/****************************************************/

/****************************************************/
/*          Populate Lookup._Authentication         */
DECLARE @h_Authentication INT

DECLARE @xmlAuthentication XML = N'
<authentications>
  <authentication Authentication="Anonymous Authentication"/>
  <authentication Authentication="ASP.NET Impersonation"/>
  <authentication Authentication="Basic Authentication"/>
  <authentication Authentication="Client Certificate Mapping Authentication"/>
  <authentication Authentication="Digest Authentication"/>
  <authentication Authentication="IIS Client Certificate Mapping Authentication"/>
  <authentication Authentication="Windows Authentication"/>
</authentications>
'

EXEC sys.sp_xml_preparedocument @h_Authentication OUTPUT, @xmlAuthentication

MERGE [Lookup].[_Authentication] AS t USING (
	SELECT p.Authentication
	FROM OPENXML(@h_Authentication, '/authentications/authentication', 1)
	WITH (Authentication VARCHAR(255)) AS p) AS s (Authentication)
ON (s.Authentication = t.Authentication)
WHEN MATCHED THEN
	UPDATE SET Authentication = s.Authentication
WHEN NOT MATCHED BY TARGET THEN
	INSERT (Authentication) VALUES (s.Authentication)
;
/*          Populate Lookup._Authentication         */
/****************************************************/

/****************************************************/
/*             Populate Lookup._Pipeline            */
DECLARE @h_Pipeline INT

DECLARE @xmlPipeline XML = N'
<pipelines>
  <pipeline ManagedPipelineMode="Classic"/>
  <pipeline ManagedPipelineMode="Integrated"/>
  <pipeline ManagedPipelineMode="Unmanaged"/>
</pipelines>
'

EXEC sys.sp_xml_preparedocument @h_Pipeline OUTPUT, @xmlPipeline

MERGE [Lookup].[_Pipeline] AS t USING (
	SELECT p.ManagedPipelineMode
	FROM OPENXML(@h_Pipeline, '/pipelines/pipeline', 1)
	WITH (ManagedPipelineMode VARCHAR(255)) AS p) AS s (ManagedPipelineMode)
ON (s.ManagedPipelineMode = t.ManagedPipelineMode)
WHEN MATCHED THEN
	UPDATE SET ManagedPipelineMode = s.ManagedPipelineMode
WHEN NOT MATCHED BY TARGET THEN
	INSERT (ManagedPipelineMode) VALUES (s.ManagedPipelineMode)
;
/*             Populate Lookup._Pipeline            */
/****************************************************/

/****************************************************/
/*             Populate Lookup._Framework            */
DECLARE @h_Framework INT

DECLARE @xmlFramework XML = N'
<frameworks>
  <framework ManagedRuntimeVersion="v2.0"/>
  <framework ManagedRuntimeVersion="v4.0"/>
</frameworks>
'

EXEC sys.sp_xml_preparedocument @h_Framework OUTPUT, @xmlFramework

MERGE [Lookup].[_Framework] AS t USING (
	SELECT p.ManagedRuntimeVersion
	FROM OPENXML(@h_Framework, '/frameworks/framework', 1)
	WITH (ManagedRuntimeVersion VARCHAR(255)) AS p) AS s (ManagedRuntimeVersion)
ON (s.ManagedRuntimeVersion = t.ManagedRuntimeVersion)
WHEN MATCHED THEN
	UPDATE SET ManagedRuntimeVersion = s.ManagedRuntimeVersion
WHEN NOT MATCHED BY TARGET THEN
	INSERT (ManagedRuntimeVersion) VALUES (s.ManagedRuntimeVersion)
;
/*             Populate Lookup._Framework            */
/****************************************************/

/****************************************************/
/*             Populate Lookup._SiteState           */
DECLARE @h_SiteState INT

DECLARE @xmlSiteState XML = N'
<sitestates>
  <sitestate State="Unknown"/>
  <sitestate State="Starting"/>
  <sitestate State="Started"/>
  <sitestate State="Stopping"/>
  <sitestate State="Stopped"/>
  <sitestate State="Pausing"/>
  <sitestate State="Paused"/>
  <sitestate State="Continuing"/>
</sitestates>
'

EXEC sys.sp_xml_preparedocument @h_SiteState OUTPUT, @xmlSiteState

MERGE [Lookup].[_SiteState] AS t USING (
	SELECT p.State
	FROM OPENXML(@h_SiteState, '/sitestates/sitestate', 1)
	WITH (State VARCHAR(255)) AS p) AS s (State)
ON (s.State = t.State)
WHEN MATCHED THEN
	UPDATE SET State = s.State
WHEN NOT MATCHED BY TARGET THEN
	INSERT (State) VALUES (s.State)
;
/*             Populate Lookup._SiteState            */
/****************************************************/
