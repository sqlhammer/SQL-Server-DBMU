/*
PROCEDURE:		[Logging].[usp_DisableConfig]
AUTHOR:			Derik Hammer
CREATION DATE:	10/16/2013
DESCRIPTION:	This procedure is used to disable Logging feature level configurations.
...
*/
CREATE PROCEDURE [Logging].[usp_DisableConfig]
	@OptionID int,
...
AS
	...
	IF NOT EXISTS (SELECT OptionID FROM [Logging].[Configs] WHERE OptionID = @OptionID)
	BEGIN
	...
	END
	-- - verify configid
	IF NOT EXISTS (SELECT ConfigID FROM [Logging].[Configs] WHERE ConfigID = @ConfigID)
		AND @ConfigID IS NOT NULL
	BEGIN
	...
	IF NOT EXISTS (SELECT ConfigName FROM [Configuration].[Configs] configs
					INNER JOIN [Logging].[Configs] bakConfigs ON bakConfigs.ConfigID = configs.ConfigID
