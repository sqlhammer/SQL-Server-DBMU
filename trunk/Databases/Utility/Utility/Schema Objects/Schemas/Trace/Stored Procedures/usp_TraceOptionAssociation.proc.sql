/*
PROCEDURE:		[Trace].[usp_TraceOptionAssociation]
AUTHOR:			Derik Hammer
CREATION DATE:	5/2/2012
DESCRIPTION:	This procedure is used to associate or disassociate an existing trace option to an existing umbrella configuration.
...
*/
CREATE PROCEDURE [Trace].[usp_TraceOptionAssociation]
	@debug BIT = 0,
	@OptionID INT,
	@ConfigID INT,
	@IsEnabled BIT = 0,
	...
	BEGIN TRANSACTION
		...
		BEGIN
			...
			BEGIN  
				...
				SELECT @LogEntry = 'Trace OptionID ' + CAST(@OptionID AS VARCHAR(10)) + ' is now associated with Configuration ID ' + CAST(@ConfigID AS VARCHAR(10)) + ' and is ' + CASE CAST(@IsEnabled AS CHAR(1)) WHEN '1' THEN 'enabled.' ELSE 'disabled.' END
		...
		BEGIN
			...
			BEGIN 
				...
				DELETE FROM [Trace].[Configs] WHERE [OptionID] = @OptionID AND [ConfigID] = @ConfigID

				SELECT @LogEntry = 'Trace OptionID ' + CAST(@OptionID AS VARCHAR(10)) + ' has been disassociated with Configuration ID ' + CAST(@ConfigID AS VARCHAR(10))
				EXEC Logging.usp_InsertLogEntry @Feature = 'Query Trace', @TextEntry = @LogEntry, @LogMode = 'LIMITED'
